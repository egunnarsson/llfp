...
{
    return ExpEq::check(e1.exp, e2.exp) && EqPtrV(e1.letStatments, e2.letStatments);
}
...
{
    return e1.op == e2.op && ExpEq::check(e1.lhs, e2.lhs) && ExpEq::check(e1.rhs, e2.rhs);
}

bool operator==(const UnaryExp& e1, const UnaryExp& e2)
{
    return e1.op == e2.op && ExpEq::check(e1.operand, e2.operand);
}

bool operator==(const LiteralExp& e1, const LiteralExp& e2)
{
    return e1.tokenType == e2.tokenType && e1.value == e2.value;
}
...
{
    return e2.identifier_ == e2.identifier_ && EqPtrV(e1.arguments_, e2.arguments_);
}
